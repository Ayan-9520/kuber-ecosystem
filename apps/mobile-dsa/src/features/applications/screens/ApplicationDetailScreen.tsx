        ...
        {tab === 'recommendations' ? (
          recommendations.isLoading ? (
            ...
            <View>
              ...
              {((recommendations.data as { lenders?: Array<{ lenderName: string; reason: string }> }).lenders ?? [])
                .slice(0, 3)
                .map((l) => (
              ...
              {((recommendations.data as { actions?: Array<{ title: string; description: string }> }).actions ?? [])
                .slice(0, 3)
                .map((a) => (
          ...
          listItems.map((item) => (
            <View key={String(item.id)} style={styles.row}>
              ...
              <View style={styles.rowBody}>
                <Text style={styles.rowTitle}>
                  {str(item.title ?? item.eventType ?? item.status ?? item.lenderName ?? item.documentTypeName ?? item.id)}
                </Text>
                <Text style={styles.rowSub}>
                  {formatDateTime((item.occurredAt ?? item.createdAt) as string)}
