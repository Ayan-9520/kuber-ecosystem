...
import { CUSTOMER_SIDEBAR_ACCOUNT_NAV } from '@/navigation/customerSidebarNav';
...
> = {
  ...
  Products: { active: 'grid', inactive: 'grid-outline', label: 'Products' },
  Support: { active: 'headset', inactive: 'headset-outline', label: 'Support' },
};
...
export function CustomerDesktopTabBar({ state, descriptors, navigation }: BottomTabBarProps) {
  const { colors } = useAppTheme();
  const { sidebarWidth } = useResponsiveLayout();
  const { user, customerId, logout } = useAuth();

  const customer = useQuery({
    queryKey: ['sidebar-customer', customerId],
  ...
  const displayName =
    ...
    str(user?.email?.split('@')[0] ?? user?.phone ?? 'Customer');
  const customerCode = str(customer.data?.customerCode);
  const kycStatus = str(customer.data?.kycStatus ?? 'PENDING');
    ...
    >
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.nav}>
          {CUSTOMER_SIDEBAR_ACCOUNT_NAV.map((item) => {
        ...
        >
          ...
          <View style={styles.accountCopy}>
            ...
            <Text style={[styles.accountSub, { color: colors.textMuted }]} numberOfLines={1}>
              {user?.phone ? maskPhone(user.phone) : 'Customer account'}
            </Text>
            <View style={[styles.tierPill, { backgroundColor: `${colors.primary}14` }]}>
              <Text style={[styles.tierText, { color: colors.primary }]}>{kycStatus}</Text>
            </View>
          </View>
