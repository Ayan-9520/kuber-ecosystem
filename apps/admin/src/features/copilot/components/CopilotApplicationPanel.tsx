import { useMutation, useQuery } from '@tanstack/react-query';
import { Sparkles } from 'lucide-react';

import { Button, Card, EmptyState, LoadingSpinner, StatCard } from '@/components/ui';
import { formatPercent } from '@/lib/utils';
import { copilotService, type CopilotApplicationAnalysis } from '@/services/copilot.service';

interface CopilotApplicationPanelProps {
  applicationId: string;
}

export function CopilotApplicationPanel({ applicationId }: CopilotApplicationPanelProps) {
  const analysis = useQuery({
    queryKey: ['copilot-application', applicationId],
    queryFn: () => copilotService.analyzeApplication(applicationId),
    enabled: false,
  });

  const feedback = useMutation({
    mutationFn: (rating: 'HELPFUL' | 'PARTIAL' | 'NOT_HELPFUL') =>
      copilotService.feedback({
        sessionId: analysis.data?.sessionId,
        entityType: 'APPLICATION',
        entityId: applicationId,
        rating,
      }),
  });

  const data = analysis.data;

  return (
    <Card
      title="AI Application Copilot"
      subtitle="Success probability, delay risk, and escalation guidance"
      actions={
        <Button variant="secondary" onClick={() => void analysis.refetch()} disabled={analysis.isFetching}>
          <Sparkles size={16} />
          {analysis.isFetching ? 'Analyzing...' : 'Run Analysis'}
        </Button>
      }
    >
      {analysis.isFetching && <LoadingSpinner />}

      {!data && !analysis.isFetching && (
        <EmptyState
          title="No application analysis"
          description="Run copilot analysis for success probability, missing information, and next steps."
        />
      )}

      {data && <CopilotApplicationResults data={data} onFeedback={(r) => feedback.mutate(r)} />}
    </Card>
  );
}

function CopilotApplicationResults({
  data,
  onFeedback,
}: {
  data: CopilotApplicationAnalysis;
  onFeedback: (rating: 'HELPFUL' | 'PARTIAL' | 'NOT_HELPFUL') => void;
}) {
  return (
    <div>
      <div className="stat-grid" style={{ marginBottom: '1rem' }}>
        <StatCard label="Success %" value={formatPercent(data.successProbability)} />
        <StatCard label="Delay Risk" value={formatPercent(data.delayRisk)} />
        <StatCard label="Approval %" value={formatPercent(data.approvalProbability)} />
        <StatCard label="Disbursal %" value={formatPercent(data.disbursalProbability)} />
      </div>

      {data.escalationRequired && (
        <p style={{ color: 'var(--color-danger)', marginBottom: '1rem', fontWeight: 600 }}>
          Escalation recommended — review delay risk and missing information.
        </p>
      )}

      {data.missingInformation.length > 0 && (
        <div style={{ marginBottom: '1rem' }}>
          <h4 className="info-item-label">Missing Information</h4>
          <ul style={{ margin: '0.5rem 0 0', paddingLeft: '1.25rem' }}>
            {data.missingInformation.map((item) => (
              <li key={item}>{item}</li>
            ))}
          </ul>
        </div>
      )}

      {data.nextBestActions.map((a) => (
        <div key={a.actionType} style={{ padding: '0.5rem 0', borderBottom: '1px solid var(--color-border)' }}>
          <strong>{a.title}</strong>
          <p className="page-subtitle">{a.description}</p>
        </div>
      ))}

      <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem' }}>
        <Button variant="ghost" onClick={() => onFeedback('HELPFUL')}>Helpful</Button>
        <Button variant="ghost" onClick={() => onFeedback('PARTIAL')}>Partial</Button>
        <Button variant="ghost" onClick={() => onFeedback('NOT_HELPFUL')}>Not helpful</Button>
      </div>
    </div>
  );
}
